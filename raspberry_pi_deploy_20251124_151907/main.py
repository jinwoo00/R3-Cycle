...
class TransactionProcessor:
    ...
    def process_transaction(self):
        ...
        try:
            ...
            # Send real-time update: Paper inserted
            if self.realtime and self.realtime.is_connected():
                self.realtime.send_ir_sensor_state(True)
                self.realtime.send_transaction_update(self.current_transaction_id, "weighing", {
            ...
            # Send real-time update: Weight reading
            if self.realtime and self.realtime.is_connected() and weight:
                self.realtime.send_weight_reading(weight)
            ...
            # Send real-time update: Metal detection
            if self.realtime and self.realtime.is_connected():
                self.realtime.send_metal_detection(metal_detected)
                self.realtime.send_transaction_update(self.current_transaction_id, "checking", {
                    "weight": weight,
                    "metalDetected": metal_detected
                })
